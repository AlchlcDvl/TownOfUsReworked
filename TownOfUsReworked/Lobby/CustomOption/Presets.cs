...
namespace TownOfUsReworked.Lobby.CustomOption
{
    public class Presets : CustomButtonOption
    {
        ...
        protected internal Presets(int id) : base(id, MultiMenu.main, "Load Preset Settings")
        {
            Do = ToDo;
        }
        ...
        {
            SlotButtons.Clear();
            SlotButtons.Add(new CustomButtonOption(1, MultiMenu.external, "Chaos", delegate { ImportPreset("Chaos"); }));
        ...
        private void ImportPreset(string presetName)
        {
            System.Console.WriteLine(presetName);

            string text;

            try
            {
                text = File.ReadAllText($"{TownOfUsReworked.Presets}{presetName}");
            }
            catch
            {
                Cancel(FlashRed);
