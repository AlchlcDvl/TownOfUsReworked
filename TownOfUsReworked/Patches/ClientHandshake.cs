...
namespace TownOfUsReworked.Patches
{
    public static class ClientHandshake
    {
        ...
        private const string TOU_VERSION = "1.0.0.8";
        ...
        public static class AmongUsClient_OnGameJoined
        {
            public static void Postfix(AmongUsClient __instance)
            {
                ...
                PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"AmongUsClient.OnGameJoined.Postfix - Am client, sending handshake");
        ...
        public static class InnerNetClient_HandleGameData
        {
            public static bool Prefix(InnerNetClient __instance, [HarmonyArgument(0)] MessageReader reader)
            {
                ...
                if (__instance.AmHost && reader.BytesRemaining > 3)
                {
                    ...
                    {
                        PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"InnerNetClient.HandleMessage.Prefix - Host recieved ToU-Rew handshake");
                        ...
                        PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"InnerNetClient.HandleMessage.Prefix - Adding {clientId} with ToU-Rew version {touVersion} to List<int>HandshakedClients");
                        HandshakedClients.Add(clientId);

                        if (touVersion != TOU_VERSION)
                        {
                            PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"InnerNetClient.HandleMessage.Prefix - ClientId {clientId} has mismatched ToU-Rew version {touVersion}. (Ours is {TOU_VERSION})");
        ...
        {
            PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"WaitForHandshake(innerNetClient, clientId = {clientId})");

            yield return new WaitForSeconds(5f);
            
            PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"WaitForHandshake() - Waited 5 seconds");

            if (!HandshakedClients.Contains(clientId))
            {
                PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"WaitForHandshake() - HandshakedClients did not contain clientId {clientId}");
                if (innerNetClient.allClients.ToArray().Any(x => x.Id == clientId))
                    innerNetClient.SendCustomDisconnect(clientId);
            }
            else
                PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"WaitForHandshake() - HandshakedClients contained clientId {clientId}");
        }
        ...
        public static class AmongUsClient_OnPlayerJoined
        {
            public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)] ClientData data)
            {
                ...
                {
                    PluginSingleton<TownOfUsReworked>.Instance.Log.LogMessage($"Am host and clientId {data.Id} sent JoinGameResponse");
        ...
        private static void SendCustomDisconnect(this InnerNetClient innerNetClient, int clientId)
        {
            ...
            messageWriter.Write($"The host has a different version of Town Of Us Reworked ({TOU_VERSION})");
            messageWriter.EndMessage();
