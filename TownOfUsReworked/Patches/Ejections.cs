    ...
    public static void Postfix(ExileController __instance, ExileController.InitProperties init)
    {
        ...
        if (GameSettings.ConfirmEjects)
        {
            if (GameSettings.EjectionRevealsRoles)
            {
                ...
                else if (PlayerLayer.GetLayers<Executioner>().Any(x => x.TargetPlayer == player && (Executioner.ExeCanWinBeyondDeath || !x.Dead)) && Executioner.ExeEjectScreen)
        ...
        else if (ClientOptions.CustomEjects)
        {
