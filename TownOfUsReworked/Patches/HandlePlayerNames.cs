// WIP replacement code
// namespace TownOfUsReworked.Patches;

// [HarmonyPatch(typeof(PlayerControl), nameof(PlayerControl.FixedUpdate))]
// public static class HandlePlayerNames
// {
...
//         NamePos ??= __instance.NameText().transform.localPosition;
//         PlayerNames[__instance.PlayerId] = __instance.Data.PlayerName;
//         ColorNames[__instance.PlayerId] = __instance.Data.ColorName.Replace("(", "").Replace(")", "");

//         if (IsInGame())
//         {
//             if (CustomPlayer.Local.Data.Role is LayerHandler localHandler)
//                 (__instance.NameText().text, __instance.NameText().color) = UpdateGameName(handler, localHandler);
//             else
//                 return;

//             (__instance.ColorBlindText().text, __instance.ColorBlindText().color) = UpdateColorblind(__instance);
//             __instance.transform.localScale = CustomPlayer.Custom(__instance).SizeFactor;
//         }
//     }
...
//             name = ColorNames.FirstOrDefault(x => x.Key == cache).Value;
...
//     public static (string, UColor) UpdateGameName(LayerHandler playerHandler, LayerHandler localHalder)
//     {
//         var distance = Vector2.Distance(localHalder.Player.transform.position, playerHandler.Player.transform.position);
//         var vector = playerHandler.Player.transform.position - localHalder.Player.transform.position;

//         if (PhysicsHelpers.AnyNonTriggersBetween(localHalder.Player.transform.position, vector.normalized, distance, Constants.ShipAndObjectsMask) && playerHandler.Player !=
//             localHalder.Player && !CustomPlayer.LocalCustom.Dead && GameModifiers.PlayerNames == Data.PlayerNames.Obstructed)
//         {
//             return ("", UColor.clear);
//         }

//         if (playerHandler.Player != localHalder.Player && !DeadSeeEverything() && !TransitioningSize.ContainsKey(playerHandler.Player.PlayerId) && playerHandler.Player.IsMimicking(out var
//             mimicked) && mimicked.Data.Role is LayerHandler layerHandler)
//         {
//             playerHandler = layerHandler;
//         }
//         else
//             return ("", UColor.clear);
...
//         if (HudHandler.Instance.IsCamoed && player != localHalder.Player && !CustomPlayer.LocalCustom.Dead)
//             name = ClientOptions.OptimisationMode ? "" : GetRandomisedName();
//         else if (GameModifiers.PlayerNames == Data.PlayerNames.NotVisible && !IsLobby())
//             name = "";
//         else if (CachedMorphs.TryGetValue(player.PlayerId, out var cache) && player != localHalder.Player && !CustomPlayer.LocalCustom.Dead)
//             name = PlayerNames.FirstOrDefault(x => x.Key == cache).Value;
//         else if (player.GetCustomOutfitType() is CustomPlayerOutfitType.Invis or CustomPlayerOutfitType.Colorblind && player != localHalder.Player && !CustomPlayer.LocalCustom.Dead)
//             name = "";
//         else
...
//         if (player.Is(LayerEnum.Mayor) && !DeadSeeEverything() && localHalder.Player != player)
//         {
...
//                 if (localHalder.Player.Is(LayerEnum.Consigliere))
//                 {
...
//                 }
//                 else if (localHalder.Player.Is(LayerEnum.PromotedGodfather))
//                 {
...
//             }
//         }
//         else if (player.Is(LayerEnum.Dictator) && !DeadSeeEverything() && localHalder.Player != player)
//         {
...
//         if (localHalder.Player.Is(LayerEnum.Consigliere) && !DeadSeeEverything())
//         {
//             var consigliere = localinfo[0] as Consigliere;
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.PromotedGodfather) && !DeadSeeEverything())
//         {
//             var godfather = localinfo[0] as PromotedGodfather;
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.PromotedRebel) && !DeadSeeEverything())
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Medic))
//         {
...
//         }
//         else if (localHalder.Player.Is(LayerEnum.Trapper))
//         {
...
//         }
//         else if (localHalder.Player.Is(LayerEnum.Retributionist))
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Arsonist) && !DeadSeeEverything())
//         {
...
//         }
//         else if (localHalder.Player.Is(LayerEnum.Plaguebearer) && !DeadSeeEverything())
//         {
//             var plaguebearer = localinfo[0] as Plaguebearer;

//             if (plaguebearer.Infected.Contains(player.PlayerId) && localHalder.Player != player)
//                 name += " <color=#CFFE61FF>ρ</color>";
//         }
//         else if (localHalder.Player.Is(LayerEnum.Cryomaniac) && !DeadSeeEverything())
//         {
...
//         }
//         else if (localHalder.Player.Is(LayerEnum.Framer) && !DeadSeeEverything())
//         {
...
//         }
//         else if (localHalder.Player.Is(LayerEnum.Spellslinger) && !DeadSeeEverything())
//         {
...
//         }
//         else if (localHalder.Player.Is(LayerEnum.Executioner) && !DeadSeeEverything())
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Guesser) && !DeadSeeEverything())
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.GuardianAngel) && !DeadSeeEverything())
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Whisperer) && !DeadSeeEverything())
//         {
//             var whisperer = localinfo[0] as Whisperer;

//             if (whisperer.Persuaded.Contains(player.PlayerId) && player != localHalder.Player)
//             {
...
//                 name += $" {value}%";
//         }
//         else if (localHalder.Player.Is(LayerEnum.Dracula) && !DeadSeeEverything())
//         {
//             var dracula = localinfo[0] as Dracula;

//             if (dracula.Converted.Contains(player.PlayerId) && player != localHalder.Player)
//             {
//                 if (GameModifiers.FactionSeeRoles && !revealed)
//                 {
//                     var role = info[0] as Role;
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Jackal) && !DeadSeeEverything())
//         {
//             var jackal = localinfo[0] as Jackal;

//             if (jackal.Recruited.Contains(player.PlayerId) && player != localHalder.Player)
//             {
//                 if (GameModifiers.FactionSeeRoles && !revealed)
//                 {
//                     var role = info[0] as Role;
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Necromancer) && !DeadSeeEverything())
//         {
//             var necromancer = localinfo[0] as Necromancer;

//             if (necromancer.Resurrected.Contains(player.PlayerId) && player != localHalder.Player)
//             {
//                 if (GameModifiers.FactionSeeRoles && !revealed)
//                 {
//                     var role = info[0] as Role;
...
//             }
//         }
//         else if (localHalder.Player.Is(Alignment.NeutralKill) && !DeadSeeEverything() && NeutralKillingSettings.KnowEachOther)
//         {
//             if (((player.GetRole().Type == Role.LocalRole.Type && NeutralSettings.NoSolo == NoSolo.SameNKs) || (player.GetAlignment() == localHalder.Player.GetAlignment() &&
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Grenadier))
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Blackmailer))
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Silencer))
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Ambusher))
//         {
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Crusader))
//         {
...
//             }
//         }
//         else if ((IsCustomHnS() || IsTaskRace()) && player != localHalder.Player)
//         {
//             name += $"\n{info[0]}";
//             color = info[0].Color;
//             revealed = true;
...
//         if (localHalder.Player.IsBitten() && !DeadSeeEverything() && !localHalder.Player.Is(LayerEnum.Dracula))
//         {
//             var dracula = localHalder.Player.GetDracula();
...
//             }
//         }
//         else if (localHalder.Player.IsRecruit() && !DeadSeeEverything() && !localHalder.Player.Is(LayerEnum.Jackal))
//         {
//             var jackal = localHalder.Player.GetJackal();
...
//             }
//         }
//         else if (localHalder.Player.IsResurrected() && !DeadSeeEverything() && !localHalder.Player.Is(LayerEnum.Necromancer))
//         {
//             var necromancer = localHalder.Player.GetNecromancer();
...
//             }
//         }
//         else if (localHalder.Player.IsPersuaded() && !DeadSeeEverything() && !localHalder.Player.Is(LayerEnum.Whisperer))
//         {
//             var whisperer = localHalder.Player.GetWhisperer();
...
//         if (localHalder.Player.Is(LayerEnum.Lovers) && !DeadSeeEverything())
//         {
//             var lover = localinfo[3] as Disposition;
//             var otherLover = localHalder.Player.GetOtherLover();
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Rivals) && !DeadSeeEverything())
//         {
//             var rival = localinfo[3] as Disposition;
//             var otherRival = localHalder.Player.GetOtherRival();
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Linked) && !DeadSeeEverything())
//         {
//             var link = localinfo[3] as Disposition;
//             var otherLink = localHalder.Player.GetOtherLink();
...
//             }
//         }
//         else if (localHalder.Player.Is(LayerEnum.Mafia) && !DeadSeeEverything())
//         {
//             var mafia = localinfo[3] as Mafia;

//             if (player.Is(LayerEnum.Mafia) && player != localHalder.Player)
//             {
...
//         if (localHalder.Player.Is(LayerEnum.Snitch) && !DeadSeeEverything())
//         {
...
//         if (player.Is(LayerEnum.Snitch) && !DeadSeeEverything() && player != localHalder.Player && (localHalder.Player.Is(Faction.Syndicate) || localHalder.Player.Is(Faction.Intruder) ||
//             (localHalder.Player.Is(Faction.Neutral) && Snitch.SnitchSeesNeutrals)))
//         {
//             var role = info[0] as Role;
...
//         if (localHalder.Player.GetFaction() == player.GetFaction() && player != localHalder.Player && player.GetFaction() is Faction.Intruder or Faction.Syndicate && !DeadSeeEverything())
//         {
//             var role = info[0] as Role;
...
//         if ((localHalder.Player.Is(Faction.Syndicate) || DeadSeeEverything()) && (player == Role.DriveHolder || (SyndicateSettings.GlobalDrive && Role.SyndicateHasChaosDrive &&
//             player.Is(Faction.Syndicate))))
//         {
//             name += " <color=#008000FF>Δ</color>";
//         }

//         if (PlayerLayer.GetLayers<Revealer>().Any(x => x.TasksDone && !x.Caught) && localHalder.Player.Is(Faction.Crew))
//         {
//             var role = info[0] as Role;
...
//         if ((DeadSeeEverything() || localHalder.Player.Is(LayerEnum.Pestilence)) && Pestilence.Infected.TryGetValue(player.PlayerId, out var count))
//         {
