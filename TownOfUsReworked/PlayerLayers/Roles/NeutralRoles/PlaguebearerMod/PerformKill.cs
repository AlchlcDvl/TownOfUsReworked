...
namespace TownOfUsReworked.PlayerLayers.Roles.NeutralRoles.PlaguebearerMod
{
    ...
    public class PerformKill
    {
        public static bool Prefix(KillButton __instance)
        {
            ...
            if (role.InfectTimer() != 0)
                return false;

            if (role.ClosestPlayer == null)
                return false;

            if (role.InfectedPlayers.Contains(role.ClosestPlayer.PlayerId))
                return false;
            ...
            unchecked
            {
                var writer2 = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.Infect,
                    SendOption.Reliable, -1);
