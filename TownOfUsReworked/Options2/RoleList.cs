...
// public class RoleListEntryAttribute() : OptionAttribute(MultiMenu2.RoleList, CustomOptionType.Entry)
// {
...
//             Alignments.Keys.ForEach(x => SlotButtons.Add(new(MultiMenu2.RoleListEntry, Alignments[x], delegate { SetVal(x); })));
//         }
//         else
//             keys.RemoveAll(x => x is LayerEnum.Crewmate or LayerEnum.Impostor or LayerEnum.Anarchist or LayerEnum.Murderer || IsAdded(x));

//         keys.ForEach(x => SlotButtons.Add(new(MultiMenu2.RoleListEntry, Entries[x], delegate { SetVal(x); })));
//         SlotButtons.Add(new(MultiMenu2.RoleListEntry, "Cancel", Cancel));
