...
//     public PresetOption() : base(MultiMenu2.Main, "Load Preset Settings") => Do = ToDo;
...
//     {
//         SlotButtons.Clear();
//         Directory.EnumerateFiles(TownOfUsReworked.Options).OrderBy(x => x).Select(x => x.SanitisePath()).Where(x => !x.EndsWith(".json")).ForEach(x => SlotButtons.Add(new(MultiMenu2.Presets,
//             x, () => LoadPreset(x))));
//         SlotButtons.Add(new(MultiMenu2.Presets, "Cancel", () => Cancel(FlashWhite, "")));
