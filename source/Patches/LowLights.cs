...
namespace TownOfUs
{
    ...
    public static class LowLights
    {
        public static bool Prefix(ShipStatus __instance, [HarmonyArgument(0)] GameData.PlayerInfo player, ref float __result)
        {
            ...
            if (player.IsImpostor() || player._object.Is(RoleEnum.Glitch) || player._object.Is(RoleEnum.Juggernaut) || player._object.Is(RoleEnum.Pestilence) || player._object.Is(RoleEnum.Arsonist) || player._object.Is(RoleEnum.Werewolf))
            {
            ...
            if (Patches.SubmergedCompatibility.isSubmerged())
            {
                ...
                if (player._object.Is(ModifierEnum.Lighter)) __result = Mathf.Lerp(__instance.MinLightRadius, __instance.MaxLightRadius, 1) * PlayerControl.GameOptions.ImpostorLightMod;
                return false;
            }
